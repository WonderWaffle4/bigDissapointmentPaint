...
//            if(words.size() != 3)
//                return false;
//            if(!isNumber(words[0]) || !isNumber(words[1]) || itc_len(words[2]) != 1)
//                return false;
//        }
//        if(lineIndex > 0){
...
bool checkLines(const vector<string> &commandsList){
    ...
    for(int lineIndex = 0; lineIndex < commandsList.size(); lineIndex++){
        if(lineIndex == 0){
            ...
            if(!isNumber(words[0]) || !isNumber(words[1]) || itc_len(words[2]) != 1 || !TrueIntFalseFloat(words[0]) || !TrueIntFalseFloat(words[1])){
                cout << isNumber(words[0]) << " " << isNumber(words[1]) << " " << itc_len(words[2]);
                return false;
            }
        }
...
//    inputFromFile.open("test.it");
//    readFromFile(commandsList, inputFromFile);
//    (checkLines(commandsList)) ? cout << endl << "Correct" : cout << endl << "Incorrect";
    return 0;
}
